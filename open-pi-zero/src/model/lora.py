...
except ImportError:  # bitsandbytes is only required for 4-bit quantization.
    BITSANDBYTES_AVAILABLE = False
    class _BitsAndBytesUnavailable:
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "bitsandbytes is required for 4-bit quantization. "
                "Install it or disable quantization for inference."
            )

    class _BitsAndBytesNamespace:
        Params4bit = _BitsAndBytesUnavailable
        Linear4bit = _BitsAndBytesUnavailable

        class functional:
            @staticmethod
            def quantize_4bit(*args, **kwargs):
                raise ImportError("bitsandbytes is required for 4-bit quantization")

    class _BitsAndBytesModule:
        nn = _BitsAndBytesNamespace
        functional = _BitsAndBytesNamespace.functional

    bnb = _BitsAndBytesModule()
...
def get_layer(
    ...
    if quantize and not BITSANDBYTES_AVAILABLE:
        raise ImportError(
            "bitsandbytes is required when quantize=True. "
            "Install it or disable quantization for inference."
        )
