...
def enable_batch_invariant_mode():
    ...
    if _batch_invariant_MODE or not torch.cuda.is_available():
        return
    # dispatch_key = getattr(torch.accelerator.current_accelerator(), "type", "cpu").upper()
    dispatch_key = "CUDA"  # Assuming CUDA for now; adjust as needed for other backends
...
def set_batch_invariant_mode(enabled: bool = True):
    """Temporarily enable or disable the CUDA dispatch overrides.

    The previous implementation restored a destroyed ``torch.library`` handle,
    which made nested contexts unreliable.  Preserve only the logical state
    and recreate registrations when a context actually changes it.
    """
